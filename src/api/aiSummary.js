...
const SUMMARY_PROMPTS = {
  professional: {
    system: "Você é um assistente profissional especializado em análise de conversas de grupos. Analise as mensagens fornecidas e crie um resumo profissional, focando em pontos importantes, decisões tomadas e ações necessárias.",
    tone: "profissional e objetivo"
  },
  casual: {
    system: "Você é um assistente amigável que analisa conversas de grupos. Crie um resumo casual e fácil de entender, destacando os principais tópicos discutidos e momentos interessantes.",
    tone: "casual e amigável"
  },
  analytical: {
    system: "Você é um analista especializado em comunicação. Analise as mensagens do grupo de forma detalhada, identificando padrões, temas principais, sentimentos predominantes e insights relevantes.",
    tone: "analítico e detalhado"
  },
  brief: {
    system: "Você é um assistente que cria resumos concisos. Analise as mensagens e forneça apenas os pontos mais importantes em formato de tópicos.",
...
router.post('/summarize', authenticateToken, async (req, res) => {
  try {
    ...
    ${includeStats ? 'Inclua no final um resumo estatístico com: total de mensagens, participantes mais ativos, horários de maior atividade.' : ''}
    ...
    const isStreaming = req.headers.accept && req.headers.accept.includes('text/stream');
    ...
    if (isStreaming) {
      ...
      res.write(`data: ${JSON.stringify({ type: 'start', totalBatches: batches.length })}\n\n`);
    }
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      if (batches.length > 1) {
        userPrompt += `\n\n(Esta é a parte ${i + 1} de ${batches.length} do total de mensagens)`;
      }
      ...
      if (isStreaming) {
        ...
        res.write(`data: ${JSON.stringify({ type: 'batch_complete', batch: i + 1 })}\n\n`);
      } else {
